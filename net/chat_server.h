...
public:
    ...
    ChatServer(const std::string& host, uint16_t port, int sub_reactor = 4, SSL_CTX* tls_ctx = nullptr)
        ...
        // 文件上传分片是海量慢 IO（写盘），单独用单线程池处理，
        // 避免挤占业务线程导致登录/聊天等请求被饿死。单线程同时保证同一文件分片按序落盘。
        file_thread_pool_ = std::make_unique<ThreadPool>(1);
        LOG_INFO << "File thread pool created with 1 thread";

        // 登录/注册等控制面请求走独立线程池，彻底与聊天、文件等数据面隔离，
        // 保证任何负载下登录都能及时响应。
        auth_thread_pool_ = std::make_unique<ThreadPool>(4);
        LOG_INFO << "Auth thread pool created with 4 threads";
        ...
        for (auto& sub : main_reactor_->getSubReactors()) {
            sub->setConnectionCreatedCallback(
                [this](int fd, std::shared_ptr<TcpConnection> conn) {
                    // 启用 TLS
                    if (tls_ctx_) {
                        conn->enableTLS(tls_ctx_);
                    }
                    // 连接关闭时清理 user_id 映射，避免留下悬空连接导致消息丢失或是误投递
                    conn->setUserCloseCallback([this](uint64_t user_id) {
        ...
        // GroupChatHandler
        groupchat_handler_.setUserConnections(&user_connections_);
        groupchat_handler_.setGroupDAO(&group_dao_);
        groupchat_handler_.setFriendDAO(&friend_dao_);
    ...
    void start() {
        ...
        //     while(running_) {
        //         // 可被 stop() 唤醒，避免关闭时 join 阻塞 10 秒
        //         stop_cv_.wait_for(lock, std::chrono::seconds(10), [this]() {
        //             return !running_;
        //         });
        //         if(!running_) {
        //             break;
        //         }
        //         lock.unlock();
        //         OnlineManager::getInstance().checkTimeout(150);
        //         lock.lock();
        //     }
        // });
    ...
    void stop() {
        ...
        if(file_thread_pool_) {
            file_thread_pool_.reset();
        }

        if(auth_thread_pool_) {
            auth_thread_pool_.reset();
        }
...
private:
    ...
    void handleMessage(std::shared_ptr<TcpConnection> conn, Buffer& buffer) {
        ...
        constexpr uint32_t MAX_PACKET_SIZE = 1024 * 1024 * 1024;

        // 本批次需要按序执行的业务消息（聊天等）；认证/文件分片即时分流，保证登录不被积压饿死
        ...
        while (buffer.readBytes() > 0) {
            ...
            if (consumed != packet_size) {
                LOG_ERROR << "Decode consumed mismatch: "
                          << "consumed=" << consumed
            ...
            auto type = header.msg_type();
            if (type == p::MSG_FILE_UPLOAD_CHUNK) {
                file_thread_pool_->enqueue([this, c = conn, h = header, b = std::move(body)]() {
                    dispatcher_.Dispatcher(c, h, b);
                });
            } else if (isAuthMessage(type)) {
                auth_thread_pool_->enqueue([this, c = conn, h = header, b = std::move(body)]() {
                    dispatcher_.Dispatcher(c, h, b);
                });
            } else {
                batch.push_back(PendingMessage{header, std::move(body)});
            }
        }

        // 业务消息按批次顺序投递到主池，保证同一批内消息的处理顺序
        if (!batch.empty()) {
            thread_pool_->enqueue([this, c = conn, batch = std::move(batch)]() {
                for (auto& m : batch) {
                    dispatcher_.Dispatcher(c, m.header, m.body);
                }
            });
        }
    }
    ...
    static bool isAuthMessage(p::MessageType type) {
        switch (type) {
            case p::MSG_REGISTER:
            case p::MSG_LOGIN:
            case p::MSG_LOGOUT:
            case p::MSG_VERIFICATION_CODE:
            case p::MSG_VERIFY_CODE_LOGIN:
    ...
    void registerHandlers() {
        ...
        dispatcher_.registerHandle(p::MSG_LOGOUT,
            [this](auto conn, auto& header, auto& body) {
                ...
                // 登出后必须清理连接映射与上下文，否则该用户仍会收到群聊/私聊消息
                if(user_id > 0) {
                    removeUserConnection(user_id);
    ...
    void onUserLogin(uint64_t user_id) {
        ...
        for(const auto& msg : group_msgs_) {
            ...
            // 与在线推送保持一致，使用 GroupMessagePush，否则客户端按字段号错位解析导致内容为空
            ...
            auto data = proto::MessageCodec::encode(msg_header, push_msg);
            if(!data.empty()) {
                conn->send(data.data(), data.size());
                sent_group++;
            }
        ...
        // 检查离线文件：在线时推送离线文件的接收列表
        FileDAO file_dao;
        auto offline_files = file_dao.getOfflineFiles(user_id);
        if (!offline_files.empty()) {
            auto conn = getConnection(user_id);
            if (conn && !conn->isClosed()) {
                db::FileOfflineListResp resp;
                resp.set_success(true);
                resp.set_total_count(static_cast<int32_t>(offline_files.size()));
                for (const auto& rec : offline_files) {
                    FileRecord file_rec;
                    if (file_dao.getFileByID(rec.file_id, file_rec)) {
                        auto* info = resp.add_files();
                        info->set_file_id(file_rec.file_id);
                        info->set_filename(file_rec.filename);
                        info->set_file_size(file_rec.file_size);
                        info->set_md5(file_rec.md5);
                        info->set_from_uid(file_rec.from_uid);
                        info->set_to_uid(file_rec.to_uid);
                        info->set_upload_time(file_rec.upload_time);
                        info->set_is_offline(true);
                    }
                }
                ...
                if (!data.empty()) {
                    conn->send(data.data(), data.size());
                    LOG_INFO << "Offline file list pushed to user " << user_id
                             << " (" << offline_files.size() << " files)";
                }
            }
        }
    ...
    std::unique_ptr<ThreadPool> auth_thread_pool_;
