...
void commandHandle(const std::string& cmd) {
    ...
    try {
        if(parts[0] == "/login" && parts.size() >= 3) {
            ...
            nick = tool::trim(nick);
            LOG_DEBUG << "register : username:" << parts[1] << ", password:" << parts[2] << ", email:" << parts[3] << ", nickname:" << nick;
            ...
            if(ui) {
                ui->setStatus("聊天中：用户 " + std::to_string(uid));
            }
        }
            ...
            if(ui) {
                ui->setStatus("聊天中：群组 " + std::to_string(gid));
            }
        }
        else if(parts[0] == "/back" || parts[0] == "/outchat") {
            client->cleanChatMod();
            if(ui) {
            ...
            std::string msg = parts.size() > 2 ? cmd.substr(cmd.find(parts[2])) : "";
